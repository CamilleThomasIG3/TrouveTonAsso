...
var FR = 'seconde_minute_heure_jour_semaine_mois_annee'.split('_');
...
var fr_FR = function fr_FR(number, index) {
  if (index === 0) return ["a l'instant", 'maintenant'];
  var unit = FR[parseInt(index / 2)];
  if (number > 1) unit += 's';
  return ["".concat(number, " ").concat(unit, " ago"), "in ".concat(number, " ").concat(unit)];
};
/**
